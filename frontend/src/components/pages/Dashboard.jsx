...
export default function Dashboard() {
	...
	useEffect(() => {
		...
		requester(
			...
			(subscriptions) =>
				setSubscriptions(
					subscriptions.active.concat(subscriptions.paused),
				),
		);
	}, []);
		...
		subscriptions.length > 0 ? (
			<>
				...
				{subscriptions[subIdx].comment && (
					<p className="px-3 my-6 italic">
						{subscriptions[subIdx].comment}
					</p>
				)}
				{!subscriptions[subIdx].comment && (
					<div className="my-6 inline-block rounded-sm border border-yellow-300 px-2 text-[.7rem] font-semibold uppercase tracking-wider text-yellow-600">
	...
	return (
		<div>
			...
			<div className="p-3 my-5 mb-20 ml-2 lg:ml-18 md:ml-12">
				<ToastContainer
				...
				<div className="mb-20">
					...
					<p className="text-sm text-slate-600">
						Pick up where you left off, start with a new
						subscription or have a look at your search history!
					</p>
				</div>
				<div className="flex justify-between gap-12 pr-6">
					<div className="relative w-[20rem] rounded border border-gray-100 px-8 pb-8 pt-20">
						...
						<div className="mt-8 space-y-3">
							<div className="mb-8">
								<label
									...
									<span>
										{userAvailableRequests} /{" "}
										{!user.isGuest ? 400 : 200}
									</span>
								</label>
								<progress
									id="file"
									className={"h-[.4rem] w-56 rounded shadow"}
									max="100"
									value={
										user
											? userAvailableRequests /
					...
					<div className="grid flex-grow grid-cols-2 grid-rows-2 gap-4 px-auto">
						...
						<Metric
							name="Comment Made"
							value={
								history.filter((h) => h.comment_id.length > 0)
									.length
							}
				...
				<div className="flex gap-4 mt-12">
					<div className="mr-12">
						...
						<div className="w-[20rem] rounded border border-gray-100 py-4 text-center">
							{currentSubscription}
						</div>
						{subscriptions.length > 0 && (
						...
						{!token.available && !token.reset && (
							<div className="flex items-baseline justify-between gap-3">
								<p className="flex-1 text-sm font-slate-400">
									We don&apos;t have auto-comment access for
									your account
								</p>
...
function Metric({ name, value, Icon }) {
	return (
		<div className="relative flex items-center justify-between px-8 py-5 border border-gray-100 rounded">
			<Icon className="absolute w-6 h-6 right-4 top-4 fill-slate-400" />
			<div>
				<p className="mb-3 text-4xl font-medium text-red-400">
					{value}
				</p>
				<p>{name}</p>
			</div>
